...
class moeadMethod(object):

    def __init__(self, objnumber, neighbour, ideaPoint):

        self.populationSize_ = int(0)
        self.obj_num = objnumber
        self.neighbour_num = neighbour
        self.ideaP = ideaPoint
        self.lambda_ = list()

    def initailize(self, objnum, weightsize):
        ideaPoint = self.initIdealPoint(objnum)
        #return as [[1,0],[0.9,0.1]]
        weights=self.initUniformWeight(objnum, weightsize)
        # not use neighbourhood table so far
        # self.initNeighbourhood(neighbourNumber)
        return weights
    ...
    def execute(self):
        ...
        # TODO:need initialise weights
        # STEP 1. Initialization
        self.initUniformWeight(self, 2)
    ...
    def initUniformWeight(self, objnum, weightsize):
        """
        ...
        """
        self.n_objectives = objnum
        # self.lambda_ = [[None] * self.n_objectives] * weightsize
        # if self.n_objectives == 2:
        # for n in range(weightsize):
        #     a = 1.0 * float(n) / (weightsize - 1)
        #     self.lambda_[n][0] = a
    ...
    """ 
    " initNeighbourhood: initailize neighbourTable
    """

    def initNeighbourhood(self, neighbourNumber):
    ...
    def initIdealPoint(self, numObjectives):
        idealpoint = [None] * numObjectives
        for i in range(numObjectives):
            idealpoint[i] = 50
        return idealpoint
        # for i in range(self.n_objectives):
        #     self.indArray_[i] = self.individual()
